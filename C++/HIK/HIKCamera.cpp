...
int HIKCamera::scanDevice()
{
	...
	if (deviceLists.nDeviceNum <= 0) {
		std::cout << "HIK: no device found" << std::endl;
	}
...
int HIKCamera::initGrab(int mode)
{
	if (mode == STEP)
	{
		//open the device
		...
		{
			int nPacketSize = 1500; //MV_CC_GetOptimalPacketSize(handle);
			if (nPacketSize > 0)
			{
...
int HIKCamera::startGrab(int mode, int triggerMode)
{
	if (mode == STEP)
	{
		/*
		...
		*/
		//start grab
		int res = MV_CC_StartGrabbing(handle);
		if (MV_OK != res)
		{
...

	{
		...
		if (triggerMode == HARD_TRIGGER) {
			res = MV_CC_SetTriggerSource(handle, MV_TRIGGER_SOURCE_LINE0);
		} 
		else {
			res = MV_CC_SetEnumValue(handle, "TriggerSource", MV_TRIGGER_SOURCE_SOFTWARE);
		}
		if (MV_OK != res)
		{
			printf("Set Trigger Mode fail! nRet [0x%x]\n", res);
...
{
	int ret = MV_CC_SetCommandValue(handle, "TriggerSoftware");
	if (MV_OK != ret) {
		std::cout << "soft trigger fail ret : " << ret;
	}
...
int HIKCamera::getDeviceInfo(std::vector<std::map<std::string, std::string>>& deviceInfo)
{
	if (deviceLists.nDeviceNum > 0)
	{
		...
		for (int i = 0;i < deviceLists.nDeviceNum;++i)
		{
			...
			unsigned char* vender = temp->SpecialInfo.stGigEInfo.chManufacturerName;
			if (strcmp((char*)vender, "Hikrobot")) {
				continue;
			}
			...
			tempMap["mac"] = macAdd;
			tempMap["type"] = "HIK";
			tempMap["model"] = (char*)temp->SpecialInfo.stGigEInfo.chModelName;
